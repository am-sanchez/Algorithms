...
// Programmers: Alejandro Sanchez, Andrew Huyhn, Jim Mao
...
int main() {
	...
	for (i = 0; i< tablesize; i++) {
		strcpy_s(t[i][0], null_st);
		strcpy_s(t[i][1], null_st);
	}
...
bool place_in_hash_tables(char *s) {
	...
	pos = f(temp_s, index); // calculate hash
	...
	while ((!placed) && (counter < 2 * tablesize)) {
		...
		if (strcmp(t[pos][index], "") == 0) {
			...
			strcpy_s(temp, t[pos][index]);
			strcpy_s(t[pos][index], temp_s);
			strcpy_s(temp_s, temp);
