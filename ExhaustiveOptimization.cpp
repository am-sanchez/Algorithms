...
// XX YY ( YOU NEED TO COMLETE YOUR NAME )
...
void print_cycle(int n, point2D* point, int* set);
...
float calcDist(int xcoord, int ycoord);
//function to caluclate distance between 2 points
...
// function to calculate the furthest distance between any two 2D points given a set of points
...
// function to calculate the distance between any two 2D points
float calcCycle( int *A, int sizeA, point2D *P)
{
	...
	dist += sqrt((P[A[sizeA-1]].x - P[A[0]].x)*(P[A[sizeA-1]].x - P[A[0]].x) + (P[A[sizeA-1]].y - P[A[0]].y)*(P[A[sizeA-1]].y - P[A[0]].y));
	return dist;  
}
	...
	// function to generate the permutation of indices of the list of points
{
	...
	if (n == 1) 
	{
		...
		cout << "\nCoords: ";
		print_cycle(sizeA, P,A);

		cout << "Dist: ";
	*/
		dist = calcCycle(A, sizeA, P);
