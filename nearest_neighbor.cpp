...
int main() {
  ...
  // set it as visited
  Visited[A] = true;
  cout << "Calculate nearest neighbor:\n";

  for(i=1; i<n; i++) {
    // calculate the nearest unvisited neighbor from node A
    B = nearest(n, P, A, Visited,max_dist);
...
int farthest_point(int n, point2D *P, float &max_dist)
// function to calculate the furthest distance between any two 2D points
//returns position of first vertex
{
	max_dist = 0;
	int i, j;
	float dist;
	...
	return  maxCoord; // index of coordinate, A
}

int nearest(int n, point2D *P, int A, bool *Visited, float & max_dist)
// function to calculate the nearest unvisited neighboring point
{
